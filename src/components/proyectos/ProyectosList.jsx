    ...
    return (
        ...
        >
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8'>
                <AnimatePresence>
                    {proyectos.slice(0, visibleProjects).map((proyecto, index) => (
                        <motion.article
                            key={proyecto.id}
                            ...
                            className={`bg-[#fbfcfc] shadow-md rounded-lg overflow-hidden transition-all duration-300 ${
                                mode ? 'light-proyecto-bg' : 'dark-proyecto-bg'
                            }`}
                            whileHover={{
                                scale: 1.01,
                        ...
                        >
                            ...
                            <div className='p-4 relative'>
                                <div className='flex items-center justify-between mb-2'>
                                    ...
                                    <motion.span 
                                        className={`text-xs font-medium text-nowrap px-2 py-1 rounded-full ${
                                            mode ? 'bg-violet-200 text-violet-300' : 'bg-violet-800 text-violet-200'
                                        }`}
                                    >
                                        {proyecto.stack}
                                    </motion.span>
                                ...
                                <div className='flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-4'>
                                    <motion.button
                                        ...
                                        className={`w-full sm:w-auto px-4 py-2 rounded-full text-white text-sm shadow-sm shadow-violet-500 ${
                                            mode ? 'bg-violet-700' : 'bg-[#9f55f2]'
                                        }`}
                                        whileHover={{ 
                                            scale: 1.05, 
                                    ...
                                    <motion.a
                                        ...
                                        className={`w-full sm:w-auto px-4 py-2 rounded-full flex items-center justify-center text-white text-sm ${
                                            mode ? 'bg-violet-700' : 'bg-[#9f55f2]'
                                        }`}
                                        whileHover={{ 
                                            scale: 1.05, 
            ...
            {visibleProjects < proyectos.length && (
                <div className="flex justify-center mt-8">
                    <motion.button
                        ...
                        className={`px-6 py-2 rounded-full transition-colors duration-300 font-semibold text-sm ${
                            mode ? 'bg-violet-700 text-white' : 'bg-[#9f55f2] text-white'
                        }`}
                        whileHover={{ 
                            scale: 1.1, 
                            boxShadow: '0px 0px 15px rgba(123, 58, 237, 0.5)',
